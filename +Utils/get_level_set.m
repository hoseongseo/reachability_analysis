function pts = get_level_set(gr, V, val)
% gr : grid structure
% V : value function defined on the grid
% val : target level

if gr.dim == 1
    pts = get_level_set1d(gr, V, val);
elseif gr.dim == 2
    pts = get_level_set2d(gr, V, val);
elseif gr.dim == 3
    pts = get_level_set3d(gr, V, val);
else
    error('Dimension larger than 3 are not supported!!')
end
end

function pts = get_level_set1d(gr, V, val)
v = gr.vs{1};
...
function pts = get_level_set2d(gr, V, val)
v1 = gr.vs{1};
v2 = gr.vs{2};
...
function pts = get_level_set3d(gr, V, val)
v1 = gr.vs{1};
v2 = gr.vs{2};
v3 = gr.vs{3};
pts = [];
