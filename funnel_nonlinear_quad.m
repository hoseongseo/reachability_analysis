function [region, cost_hist, rate_hist] = funnel_nonlinear_quad(sys, tk, Q0, args)
% sys: dynamics
...
N = 2*ones(1,sys.Nx); % target order is determined in this case
...
for iter = 1:args.max_iter
    ...
    cvx_begin
        ...
        subject to
            ...
            for k = 1:length(tk)-1
                for m = 1:prod(Nw+1)
                    B_NNx\T_NNx(:,:,k)*( C_N_Nx*E_N*(c_tilde(:,k+1)-c_tilde(:,k)) + (tk(k+1)-tk(k))*H(:,:,m,k)*E_N*c_tilde(:,k) ) <= 0
                end
            end
    cvx_end
    ...
    if args.plot_cost
        ...
        figure(213)
        cla; hold on; grid on;
        for k = round(linspace(1,length(tk),11))
