classdef MultirotorYNominal < Dynamics.Base
    properties (GetAccess = public, SetAccess = private)
        xN
        tN
    end
    methods
        function this = MultirotorYNominal(x0, u, t)
            this = this@Dynamics.Base(4,3,1);
...
%                   x(5,:);...
%                   x(6,:);...
%                   u(1)*cos(x(7,:)).*sin(x(8,:)) + w(1);...
%                   -u(1)*sin(x(7,:)) + w(2);...
%                   u(1)*(cos(x(7,:)).*cos(x(8,:))) - 9.8 + w(3);...
%                   u(2)*ones(1,size(x,2));...
%                   u(3)*ones(1,size(x,2))];
...
%             cp = 1;
              sp = x(3);
%               sp = x(3) - (1/6)*x(3)*x(3)*x(3);
%             st = x(4) - (1/6)*x(4)*x(4)*x(4);
%             st = x(4);
