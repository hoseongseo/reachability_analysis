function [region, cost_hist, rate_hist, ctime_hist] = funnel_nonlinear_lp4(sys, t, Q0, N, N_lambda, N_gamma, args)
% sys: dynamics
...
quad_mat_idx = Utils.getQuadIdx( quad_degree(quad_idx_2nd_order,:) );
...
% E_quad_N_max = Polynomial.expand_matrix(N_max, [quad_degree, zeros(size(quad_degree,1),1)]);
...
% MAIN LOOP
num_elem = length(t); % number of elements per segment
...
%                     cost = cost - h_poly'*(T_N(:,:,idx_k)*c_seg(:,k));
                end
                minimize cost
                ...
                subject to
                    ...
                    for k = 1:num_elem-1
                        ...
                        B_N\T_N(:,:,idx_k+1)*( E_quad_poly*c_quad_seg(:,k+1) - c_seg(:,k+1) ) <= 0
                        
%                         B_N_max\T_N_max(:,:,idx_k)*( E_quad_N_max*(c_quad_seg(:,k+1)-c_quad_seg(:,k)) +...
%                             (t(idx_k+1)-t(idx_k))*(E_N_N_f*M(:,:,idx_k)*E_quad_poly*c_quad_seg(:,k) -...
%                             E_N_N_lambda*P_N_N_lambda*kron(eye(prod(N+1)),lambda(:,idx_k))*E_quad_poly*c_quad_seg(:,k)) ) <= 0
...
%                 
% %                 B_N_max\T_N_max(:,:,k)*( E_quad_N_max*(c_quad(:,k+1)-c_quad(:,k)) +...
% %                             (t(k+1)-t(k))*(E_N_N_f*M(:,:,k)*E_quad_poly*c_quad(:,k) -...
% %                             E_N_N_lambda*P_N_N_lambda*kron( E_quad_poly*c_quad(:,k), eye(prod(N_lambda+1)) )*lambda(:,k)) ) <= 0
%                         
...
%     c_quad_conv = c_quad.*repmat( ones((sys.Nx)*(sys.Nx+1)/2+1,1) - 0.5*quad_idx_skew, [1,length(t)] );
    r = c_quad(1,:);
    vechA = c_quad(quad_idx_2nd_order, :);
    
    % domain to be updated
    region_new = region_;
    for k = 1:length(t)
        r_ = r(k);
        Atmp_ = zeros( sys.Nx, sys.Nx );
        Atmp_( sys.Nx*(quad_mat_idx(:,1)-1) + quad_mat_idx(:,2) ) = vechA(:,k);
        A_ = 0.5*(Atmp_ + Atmp_');
...
% 
%             tmp = sys.xN(1:2,k) + region(1).Q(1:2,1:2,k)^(1/2) * Utils.Sphere(1,200).x;
...
%             
% %             Vp = Polynomial(c(:,k), Polynomial.integer_grid(N));
% %             zeroset = Utils.get_level_set( args.grid, Vp.eval(args.grid), 0.0 );
% %             tmp = sys.xN(1:2,k) + zeroset(1:2,:);
% %             K = convhull(tmp(1,:)', tmp(2,:)');
% %             plot(tmp(1,K), tmp(2,K), 'r', 'linewidth', 2)
%             
%             tmp = sys.xN(1:2,k) + region_new.Q(1:2,1:2,k)^(1/2) * Utils.Sphere(1,200).x;
