...
for k = 1:length(t)-1
    ...
    xip_ = fp(t(k)) - (f0_ + A0_*(xp - q_) + D0_*(wp));
...
%% Level-set method
% dynamics
sys0 = Dynamics.LotkaVolterraContNominal(q, t); % nominal dynamics with the initial condition
sys = Dynamics.LotkaVolterraCont(sys0, wMax); % system shifted to the origin
...
for i = 1:length(t)-1
    ...
    plot(X{i+1}(1,:), X{i+1}(2,:), 'k*')
    x_ = Q_proposed(:,:,i+1)^(1/2) * Math.Sphere(1,200).x;
    plot(x_(1,:), x_(2,:), 'color', 'r', 'linewidth', 2);
    title(['$t = ',num2str(t(i+1)),'$ [s]'])
    drawnow
end
